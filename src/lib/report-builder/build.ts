...
export function buildReport(spec: ReportSpec, ctx: BuildContext): BuiltReport {
  ...
  return {
    ...
    // the record list and the headline figures are switched off here rather than at the point of
    // printing, so the PDF, the Excel, the Word summary and the screen all leave out the same parts
    columns: spec.blocks.table ? columns : [],
    rows: spec.blocks.table ? rows : [],
    groups: spec.blocks.table ? groups : null,
    groupLabel: spec.blocks.table ? groupLabel : null,
    totals: spec.blocks.table ? totals : {},
    totalKeys: spec.blocks.table ? totalKeys : [],
    kpis: spec.blocks.kpis ? kpis : [],
    breakdown,
    ageing,
