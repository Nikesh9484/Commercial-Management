...
export async function renderBuilderWord(r: BuiltReport): Promise<Buffer> {
  // a report whose record list is switched off carries no columns, and gets no detail section at all
  const hasDetail = r.columns.length > 0;
  const wide = r.columns.length > 6;
  const body = [...titleBlock(r), ...executiveSummary(r), ...figures(r), ...(hasDetail && !wide ? detailTable(r, PORTRAIT_W) : []), ...basisBlock(r)];
...
/**
 * The numbers on the headings. Any section can be switched off in the report set-up, so they are
 * worked out from what the report is actually carrying rather than assumed to all be there.
 */
function sectionNumbers(r: BuiltReport) {
  let n = 2; // 1 is always the executive summary
  const position = r.narrative.length ? n++ : null;
  const analysis = r.breakdown?.bands.length || r.ageing?.bands.length ? n++ : null;
  const detail = r.columns.length ? n++ : null;
  return { position, analysis, detail, basis: n };
}

function figures(r: BuiltReport): (Paragraph | Table)[] {
  const out: (Paragraph | Table)[] = [];
  const no = sectionNumbers(r);
  if (no.position) {
    out.push(h1(`${no.position}.  The position in detail`));
    r.narrative.forEach((p, i) => {
      out.push(h2(`${no.position}.${i + 1}  ${p.heading}`));
      out.push(body(p.text));
    });
  }
  if (no.analysis) {
    out.push(h1(`${no.analysis}.  Analysis`));
    if (r.breakdown?.bands.length) {
      out.push(h2(`${no.analysis}.1  By ${r.breakdown.label.toLowerCase()}`));
      out.push(bandTable(r.breakdown.bands, PORTRAIT_W));
      out.push(spacer());
    }
    if (r.ageing?.bands.length) {
      out.push(h2(`${no.analysis}.${r.breakdown?.bands.length ? 2 : 1}  Ageing`));
...
function detailTable(r: BuiltReport, width: number): (Paragraph | Table)[] {
  const out: (Paragraph | Table)[] = [h1(`${sectionNumbers(r).detail}.  Detail`)];
...
function basisBlock(r: BuiltReport): Paragraph[] {
  const out = [h1(`${sectionNumbers(r).basis}.  Basis of preparation`)];
