...
export const bonds: RegisterDef = {
  ...
  fields: [
    ...
    { key: "days_to_expiry", label: "Days to expiry", type: "number", virtual: true, readonly: true, hideInForm: true, help: "Amber within 60 days, red within 30 days or expired. Not flagged when the contract is closed." },
    { key: "status", label: "Status", type: "text", virtual: true, readonly: true, hideInForm: true, chip: true, help: "Active, Expiring, Expired; Released when the contract is closed (Final Account Status closed / not required, contract Closed / Completed, or the box below ticked); Superseded when a newer policy of the same type is held for the same contract." },
    { key: "contract_closed", label: "Contract closed – bond released", type: "boolean", defaultValue: false, filter: true, hideInTable: true, help: "Tick when the contract is closed and the bond / policy is no longer required. Filled automatically on import from the Excel Final Account Status; the dashboard also reads it from the Final Account Status and Contracts registers." },
    { key: "released", label: "Released / no longer required", type: "boolean", virtual: true, readonly: true, hideInForm: true, hideInTable: true, filter: true, help: "Worked out by the dashboard: the row is ticked as closed, or every contract of this contractor / cost line is closed in the Final Account Status." },
    { key: "link_note", label: "Why this is still listed", type: "text", virtual: true, readonly: true, hideInForm: true, hideInTable: true, help: "Empty when the bond is released. Otherwise it says what is keeping the row on the chase list – usually a cost report line the Final Account Status does not mention, or a contractor that still has an open contract." },
    { key: "superseded", label: "Superseded by a later bond", type: "boolean", virtual: true, readonly: true, hideInForm: true, hideInTable: true, filter: true, help: "A later bond or policy of the same type covers this contract." },
