...
export function EmailReportButton() {
  ...
  const load = async () => {
    ...
    if (summary) return;
    try {
      const r = await fetch("/api/email-report?format=json");
  ...
  return (
    <>
      <button type="button" className="btn btn-primary btn-sm" onClick={load} title="Prepare a short summary email with the Executive Summary, Level 1 and Level 2 PDFs attached">
        <Mail size={14} /> Email the Report
      </button>
      <Modal open={open} title="Email the Report" onClose={() => setOpen(false)} size="lg"
        footer={
          <>
            ...
            <a className={`btn btn-primary btn-sm ${summary ? "" : "pointer-events-none opacity-50"}`} href="/api/email-report?format=eml" download title="Downloads an Outlook draft with the three PDFs attached">
        ...
        {summary && (
          <div className="space-y-3">
            <div className="rounded-xl border border-line bg-slate-50 p-3 text-xs text-ink">
              ...
              <div className="mt-1"><span className="font-semibold text-muted">Attachments:</span> Executive Summary.pdf · Cost Report Level 1.pdf · Cost Report Level 2.pdf</div>
            </div>
            ...
            <div className="rounded-xl border border-sky-200 bg-sky-50 p-3 text-xs text-sky-900">
              <b>How it works.</b> <b>Open in Outlook</b> downloads a small draft file (.eml). Open the downloaded file and Outlook shows this email, with the three PDFs attached, ready for you to check and press Send. On a phone use <b>Open mail app</b> (text only) or <b>Copy summary</b> and paste it into a new email, then attach the PDFs downloaded with the red PDF buttons.
            </div>
          </div>
        )}
      </Modal>
