...
export async function GET(req: Request, ctx: unknown) {
  return withUser(async (user) => {
    ...
    const summary = buildEmailSummary(data, { name: user.name, email: user.email });
    ...
    const attachments = [await pdf("exec", "Executive_Summary"), await pdf("level1", "Cost_Report_Level_1"), await pdf("level2", "Cost_Report_Level_2")];
